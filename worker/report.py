...
def render_report(bundle, synthetic=False, prompt_loop_summary=None):
    ...
    story.append(p("Optional dashboard focus timing: " + ("selected" if session["consent"].get("focus_events") else "not selected") + ". Client-reported focus events are unscored; destinations and content are not collected."))
